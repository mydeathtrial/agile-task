...
public class TaskManager implements ApplicationRunner {
	...
	public void updateTask(Task task) throws NotFoundTaskException, NoSuchMethodException {
		...
		for (String cron : crones) {
			...
			} else if (cron.startsWith(FIXED_RATE)) {
				cron = cron.substring(FIXED_RATE.length());
				long executeTime = NumberUtils.toLong(cron);
				Instant instant = Instant.ofEpochMilli(executeTime);
				if (task.getEnable() != null && task.getEnable()) {
					scheduledFuture = threadPoolTaskScheduler.scheduleAtFixedRate(job, executeTime);
				}
				actuators.add(new FixedRateActuator(scheduledFuture, instant, threadPoolTaskScheduler));
			} else if (cron.startsWith(FIXED_DELAY)) {
				cron = cron.substring(FIXED_DELAY.length());
				long executeTime = NumberUtils.toLong(cron);
				Instant instant = Instant.ofEpochMilli(executeTime);
				if (task.getEnable() != null && task.getEnable()) {
					scheduledFuture = threadPoolTaskScheduler.scheduleWithFixedDelay(job, executeTime);
				}
				actuators.add(new FixedDelayActuator(scheduledFuture, instant, threadPoolTaskScheduler));
			} else {
				//新建定时任务触发器
